...
- [Supadata](https://supadata.ai/)（字幕；繞過雲端 IP 封鎖，無字幕影片自動轉寫）＋ `youtube-transcript`（本機回退）
- YouTube oEmbed（影片資訊，免金鑰）
- Vitest（單元測試）
...
| `SUPADATA_API_KEY` | [Supadata](https://supadata.ai/) 金鑰（字幕來源，免費 100 次/月） | 部署到雲端建議填 |

> **關於字幕來源**：設定 `SUPADATA_API_KEY` 後優先用 Supadata 抓字幕——這能繞過雲端主機（如 Vercel）常被 YouTube 封鎖 IP 的問題，且無字幕影片會自動用 Whisper 轉寫。未設定則改用 `youtube-transcript`（本機可用，但部署到雲端常抓不到字幕）。
...
2. 後端並行抓取影片資訊（oEmbed）與字幕（Supadata 優先，回退 `youtube-transcript`）
